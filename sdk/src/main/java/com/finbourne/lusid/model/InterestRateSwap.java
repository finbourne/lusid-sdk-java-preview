...
import com.finbourne.lusid.model.InterestRateSwapAllOf;
 ...
 */
@ApiModel(description = "LUSID representation of an Interest Rate Swap, including:      * Vanilla (single currency fixed-float non-amortising)    * CrossCurrency (>1 currency is used by the swap legs)    * Basis (single currency, floating-floating legs of different tenors)    * Amortising (the swap has 1+ leg with amortised notional)")
...
public class InterestRateSwap extends LusidInstrument {
  ...
  private List<AdditionalPayment> additionalPayments = null;
  ...
  **/
  @ApiModelProperty(value = "Is the contract an IRS of \"Non-Deliverable\" type, meaning a single payment in the settlement currency based on the difference between  the fixed and floating rates.")
   ...
   * @return legs
  **/
  @ApiModelProperty(required = true, value = "The set of instrument legs that define the swap instrument, these should be FloatingLeg or FixedLeg.")
  ...
  **/
  @ApiModelProperty(value = "Settlement currency if IRS is non-deliverable.")
  ...
  **/
  @ApiModelProperty(value = "Optional additional payments at a given date e.g. to level off an uneven fixed-floating swap.  The dates must be distinct and either all payments are Pay or all payments are receive")
